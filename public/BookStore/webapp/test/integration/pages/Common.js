sap.ui.define([
	...
	function getFrameUrl(sHash, sUrlParameters) {
		var sUrl = jQuery.sap.getResourcePath("pottmeier/bookstore/app", ".html");
		sHash = sHash || "";
		sUrlParameters = sUrlParameters ? "?" + sUrlParameters : "";

		if (sHash) {
			sHash = "#BookStore-display&/" + (sHash.indexOf("/") === 0 ? sHash.substring(1) : sHash);
		} else {
			sHash = "#BookStore-display";
		}

		return sUrl + sUrlParameters + sHash;
	}

	return Opa5.extend("pottmeier.bookstore.test.integration.pages.Common", {
		...
		getMockServer: function() {
			return new Promise(function(success) {
				Opa5.getWindow().sap.ui.require(["pottmeier/bookstore/localService/mockserver"], function(mockserver) {
